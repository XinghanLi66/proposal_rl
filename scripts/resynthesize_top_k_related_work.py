#!/usr/bin/env python3
"""
Parallel re-synthesis of TopKRelatedWork prompts.

Reads every record from runs/dataset/train.jsonl, calls TopKRelatedWorkBuilder.build()
for each one (which writes to the new top_k_{k}_index.jsonl and
top_k_{k}_related_work.jsonl caches), and skips records already present in
the new cache.

Usage:
    python scripts/resynthesize_top_k_related_work.py [--workers N] [--k K] [--config PATH]
...
# Proxy credentials (mirror _combined_lib.sh defaults)
...
def build_one(builder, record: dict) -> tuple[str, bool, str]:
    ...
    try:
        ...
        cached = elapsed < 0.05  # cache hit is near-instant
...
def main():
    ...
    # Check existing new-format cache to determine how many are already done
    new_rw_cache = cache_dir / f"top_k_{args.k}_related_work.jsonl"
    already_done: set[str] = set()
    if new_rw_cache.exists():
        with open(new_rw_cache) as f:
            for line in f:
                try:
                    d = json.loads(line)
    ...
    # Build per-thread builder instances (each has its own _Cache in-memory dict,
    # but they all append to the same file through fcntl locking).
    pb_cfg = cfg.get("prompt_builder", {}).copy()
    pb_cfg.setdefault("runs_dir", runs_dir)
    pb_cfg["top_k"] = args.k
    pb_cfg["strategy"] = "top_k_related_work"

    from train.prompt_builder import TopKRelatedWorkBuilder

    def make_builder():
        return TopKRelatedWorkBuilder(pb_cfg)

    builders = [make_builder() for _ in range(args.workers)]
